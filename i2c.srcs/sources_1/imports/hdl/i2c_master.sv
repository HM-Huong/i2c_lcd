...
module i2c_master(
   ...
   input  logic [2:0] cmd, 
   input  logic wr_cmd, // 1 for write command
   output tri scl,
   inout  tri sda,
   output logic ready, // ready to write/read 1 byte
   output logic done_tick, // rising edge after wirte/read 1 bit
   output logic ack, // 0 for ack, 1 for nack
   ...
   begin
      ...
      case (state_reg)
         idle: begin
            ready_i = 1'b1;
            if (wr_cmd && cmd==START_CMD) begin  //start 
            	ready_i = 1'b0;
               state_next = start1;
               c_next = 0;
            end
         ...
         data2: begin
            ...
            if (c_reg==qutr) begin
               c_next = 0;
               state_next = data3;
               rx_next = {rx_reg[7:0], sda}; //shift data in at then end of data2
            end // end if
         end
         data3: begin
            sda_out = tx_reg[8];
